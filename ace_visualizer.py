...
class ACEVisualizer:
    ...
    def __init__(
        ...
        self.framecount_transition = (
            40  # frame count for growing the fully trained map at the end of mapping
        )
        ...
        self.reloc_color_map = plt.cm.get_cmap("summer")(np.linspace(0, 1, 256))[
            1:, :3
        ]  # green to yellow
    ...
    def _generate_camera_trajectory(self, mapping_poses, mapping_image_paths):
        ...
        for frame_idx in range(len(mapping_poses)):
            ...
            self.trajectory_buffer.add_camera_frustum(
                frustum_pose, image_file=mapping_image_paths[frame_idx], sparse=True
            )
    ...
    def setup_mapping_visualisation(
        self, pose_files, image_files, frame_count, camera_z_offset
    ):
        """
        ...
        mapping_poses = [
            self._convert_cv_to_gl(np.loadtxt(pose_file)) for pose_file in pose_files
        ]
    ...
    def _draw_hist(
        self, image, hist_values, hist_colors, hist_x, hist_y, hist_w, hist_h, hist_max
    ):
        """
    ...
    def _write_reloc_captions(self, image):
        ...
        captions_dict = [
            {"x": 0.32, "y": 0.25, "fs": 0.07 * image_h, "text": "Relocalisation"},
            {
                "x": 0.11,
                "y": 0.15,
                "fs": 0.04005 * image_h,
    ...
    def setup_reloc_visualisation(
        self, frame_count, network, data_loader, camera_z_offset, reloc_frame_skip
    ):
        """
    ...
    def render_reloc_frame(
        self, query_pose, query_file, est_pose, est_error, sparse_query
    ):
        """
        ...
        self.trajectory_buffer.add_camera_frustum(
            query_pose, image_file=query_file, sparse=False
        )
        self.trajectory_buffer.add_camera_frustum(
            est_pose, image_file=None, sparse=False, frustum_color=err_color
        )
