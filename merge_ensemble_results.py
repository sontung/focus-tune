...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=".txt",
        help="Suffix to select a subset of pose files.",
    )
    ...
    for in_file in pose_files:
        ...
        with in_file.open("r") as f:
            for line in f.readlines():
                ...
                (
                    img,
                    current_result.quaternion[0],
                    current_result.quaternion[1],
                    current_result.quaternion[2],
                    current_result.quaternion[3],
                    current_result.translation[0],
                    current_result.translation[1],
                    current_result.translation[2],
                    current_result.r_err,
                    current_result.t_err,
                    current_result.inlier_count,
                ) = line.split()

                # Convert to the appropriate datatypes.
                current_result.inlier_count = int(current_result.inlier_count)
                current_result.quaternion = [
                    float(x) for x in current_result.quaternion
                ]
                current_result.translation = [
                    float(x) for x in current_result.translation
                ]
